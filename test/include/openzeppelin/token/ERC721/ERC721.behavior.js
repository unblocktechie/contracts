...
function shouldBehaveLikeERC721(
  ...
  context('with minted tokens', function () {
    beforeEach(async function () {
      await this.token.mint(owner, true)
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expectRevert(
            ...
            'ERC721X: balance query for the zero address',
          )
        })
      })
    })
        ...
        function () {
          ...
          it('reverts', async function () {
            await expectRevert(
              ...
              'ERC721X: owner query for nonexistent token',
            )
          })
        },
      )
          ...
          function () {
            it('reverts', async function () {
              await expectRevert(
                ...
                'ERC721X: transfer caller is not owner nor approved',
              )
            })
          },
        )
        ...
        context('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await expectRevert(
              ...
              'ERC721X: operator query for nonexistent token',
            )
          })
        })
        ...
        function () {
          it('reverts', async function () {
            await expectRevert(
              ...
              'ERC721X: approval to current owner',
            )
          })
        },
      )
      ...
      context('when the sender does not own the given token ID', function () {
        it('reverts', async function () {
          await expectRevert(
            ...
            'ERC721X: approve caller is not owner nor approved',
          )
        })
      })
      ...
      context('when token is not minted', async function () {
        it('reverts', async function () {
          await expectRevert(
            ...
            'ERC721X: approved query for nonexistent token',
          )
        })
      })
  ...
  describe('_mint(address, bool)', function () {
    it('reverts with a null destination address', async function () {
      await expectRevert(
        this.token.mint(ZERO_ADDRESS, false),
        'ERC721X: mint to the zero address',
      )
    })

    context('with minted token', async function () {
      beforeEach(async function () {
        ;({ logs: this.logs } = await this.token.mint(owner, false))
      })
    ...
    context('with minted token with extra token', async function () {
      beforeEach(async function () {
        ;({ logs: this.logs } = await this.token.mint(owner, true))
      })

      it('emits a Transfer event for the first token minted', function () {
        expectEvent.inLogs(this.logs, 'Transfer', {
          from: ZERO_ADDRESS,
          to: owner,
          tokenId: firstTokenId,
      ...
      it('emits a Transfer event for the second token minted', function () {
        expectEvent.inLogs(this.logs, 'Transfer', {
          from: ZERO_ADDRESS,
          to: owner,
      ...
      it('creates the first token minted', async function () {
        expect(await this.token.ownerOf(firstTokenId)).to.equal(owner)
      })

      it('creates the second token minted', async function () {
        expect(await this.token.ownerOf(secondTokenId)).to.equal(owner)
      })

      it('should correctly increase owner token count', async function () {
        expect(await this.token.balanceOf(owner)).to.be.bignumber.equal('2')
      })
    })
  })
    ...
    describe('tokenOfOwnerByIndex', function () {
      describe('when the given index is lower than the amount of tokens owned by the given address', function () {
        it('returns the token ID placed at the given index', async function () {
          expect(
            await this.token.tokenOfOwnerByIndex(owner, 0),
          ).to.be.bignumber.equal(firstTokenId)
        })
      })
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await expectRevert(
            ...
            'ERC721X: owner index out of bounds',
          )
        })
      })
    ...
    describe('tokenByIndex', function () {
      ...
      it('reverts if index is greater than supply', async function () {
        await expectRevert(
          ...
          'ERC721X: global index out of bounds',
        )
      })
      ...
      //     await this.token.mint(newOwner, true)
  ...
  describe('_mint(address, bool)', function () {
    ...
    context('with minted token', async function () {
      ...
      it('adjusts all tokens list', async function () {
        expect(await this.token.tokenByIndex(0)).to.be.bignumber.equal(
          firstTokenId,
        )
      })
    })
    ...
    context('with minted token with extra token', async function () {
      ...
      it('adjusts owner tokens by index for first token', async function () {
        expect(
          await this.token.tokenOfOwnerByIndex(owner, 0),
        ).to.be.bignumber.equal(firstTokenId)
      })

      it('adjusts owner tokens by index for second token', async function () {
        expect(
          await this.token.tokenOfOwnerByIndex(owner, 1),
        ).to.be.bignumber.equal(secondTokenId)
      })

      it('adjusts all tokens list second first token', async function () {
        expect(await this.token.tokenByIndex(1)).to.be.bignumber.equal(
          secondTokenId,
        )
      })
    })
...
function shouldBehaveLikeERC721Metadata(errorPrefix, name, symbol, owner) {
  ...
  describe('metadata', function () {
    ...
    describe('token URI', function () {
      beforeEach(async function () {
        await this.token.mint(owner, false)
      })
